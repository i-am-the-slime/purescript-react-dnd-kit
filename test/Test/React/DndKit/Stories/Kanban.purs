...
import React.DndKit.Types (DragDropManager, DragEndEvent, DraggableId(..), DroppableId(..), Source, Target, callbackRef, move)
...
kanban = component "Kanban" \props -> React.do
  ...
  let
    onDragEnd :: DragEndEvent -> DragDropManager -> _
    onDragEnd event _ = setColumns \current ->
      handleDrag current event.operation.source event.operation.target
  pure $ div { style: Styles.boardStyle } do
    dragDropProvider { onDragEnd }
      [ column { name: "To Do", group: "todo", tasks: columns.todo, columnColor: props.columnColor, cardColor: props.cardColor }
      , column { name: "Doing", group: "doing", tasks: columns.doing, columnColor: props.columnColor, cardColor: props.cardColor }
      , column { name: "Done", group: "done", tasks: columns.done, columnColor: props.columnColor, cardColor: props.cardColor }
      ]
  where
  initialColumns =
    { todo:
...
handleDrag cols (Just src) (Just tgt) = do
  ...
  case findColumn sourceId cols /\ findColumn targetId cols of
    ...
    Just fromCol /\ Nothing
      | isColumnName targetId -> moveBetweenColumns cols fromCol targetId sourceId targetId
    _ -> cols
handleDrag cols _ _ = cols

isColumnName :: String -> Boolean
isColumnName "todo" = true
isColumnName "doing" = true
isColumnName "done" = true
  ...
  pure $
    div { style: Styles.columnStyle props.columnColor }
      [ div { style: Styles.headerStyle }
          [ h3 { style: Styles.titleStyle } (text props.name)
